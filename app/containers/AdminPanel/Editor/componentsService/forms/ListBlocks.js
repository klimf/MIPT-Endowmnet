...
function Item(name, index, fields) {
  return (
    <Wrapper>
      ...
      <DeleteButton
        label="Удалить" style={{ position: 'static', margin: '12px 0' }}
    ...
    <div>
      ...
      <NegPadding padding={24}>
        <FlexBox>
          ...
          <Column padding="0 24px" small={12} medium={6} large={4}>
            <ListWrapper>
              <div>{}</div>
              <ul>
                <li>{}</li>
      ...
      <Title>
        <Column all={6}>
          <Field
            validate={[required]} name={`${name}.title`}
            component={(props) => (<TextInput options={{ fullWidth: true }} {...props} />)} label="Загловок"
          />
        </Column>
      </Title>
      <Space size={2} />
